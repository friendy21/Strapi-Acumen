/**
 * Author Lifecycle Hooks
 * =============================================================================
 * Handles tenant isolation for all Author CRUD operations.
...
module.exports = {
    // Inject tenant_id before creating new authors
    async beforeCreate(event) {
        const tenantId = getTenantId(strapi);
        if (!tenantId || tenantId === 'default') {
            strapi.log.warn(`[Tenant] Creating author without explicit tenant_id`);
        }
        event.params.data.tenant_id = tenantId;
    },

    // Filter by tenant_id before finding single author
    async beforeFindOne(event) {
        const tenantId = getTenantId(strapi);
        injectTenantFilter(event, tenantId);
    },

    // Filter by tenant_id before finding multiple authors
