 ...
 * =============================================================================
 * Handles tenant isolation for all Article CRUD operations.
...
module.exports = {
    // Inject tenant_id before creating new articles
    async beforeCreate(event) {
        const tenantId = getTenantId(strapi);
        if (!tenantId || tenantId === 'default') {
            strapi.log.warn(`[Tenant] Creating article without explicit tenant_id`);
        }
        event.params.data.tenant_id = tenantId;
    },

    // Filter by tenant_id before finding single article
    async beforeFindOne(event) {
        const tenantId = getTenantId(strapi);
        injectTenantFilter(event, tenantId);
    },

    // Filter by tenant_id before finding multiple articles
